...
def execute_scan_task(scan_id: str, project_id: str, db_session_factory):
    # Retrieve a database session in background thread
    with next(db_session_factory) as db:
...
def trigger_scan(background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    background_tasks.add_task(execute_scan_task, scan_id, settings.GCP_PROJECT_ID, get_db)
    return scan
